...
/* 6.  which most favourite product of each customer and how many times they ordered it ? */
    ...
    select t.userid,t.product_id,p.product_name,t.Times, (t.times*p.price) as total_cost from 
(
with cte as (select userid, product_id , count(product_id) as Times  from sales 
...
/* 10.  If buying each product generates points for example $5 = 2 zomato points and each product has different purchasing points like for p1 ,  $5 = 1point ,
...
/* 11.  This question is like above question but in this we going to find out  
...
/* 12.   In the first one year after a customer joins the gold program (including their join date ) irrespective of what the customer 
        has purchased they earn 5 zomato points for every $10 spent. Who earned more and what was their points earning in their first year ?*/
        
        with cte as ( select s.userid,s.product_id, s.created_date ,  g.gold_signup_date, row_number() over(partition by s.userid order by s.product_id) as rnk from sales s 
                       inner join goldusers_signup g
					   ...
					   order by s.userid),
            cte2 as (select cte.userid AS UserID , cte.product_id as ProductID, p.price as Price,p.product_name AS ProductName from cte 
                       left join product p 
					   on cte.product_id = p.product_id)
                       
        select UserID , ProductID , ProductName , Price,  Round(Price / 2) as Points from cte2;
        


/*  Note :- how to use Date_add function */

        select * , DATE_ADD(gold_signup_date,interval 1 year) AS v
...
/* 13.  Rank all the transaction of the customers ?*/   
        ...
        select * , Rank() over(partition by userid order by created_date) as Transaction_Rank from sales ;  
 


 /* 14.  Rank all the transaction for each customer whenever they are a gold member and for every non-gold member transaction mark as "NA". */ 
 
        select s.userid,s.product_id ,s.created_date,
                   case when g.gold_signup_date IS NULL then "NA" 
			      else rank() over(partition by s.userid order by s.created_date desc) end as RNK from sales s
